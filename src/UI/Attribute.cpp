...
void Attribute::ComponentVisitor::ImplVisit(Ptr<TriMesh> mesh) {
	...
	grid->AddButton("ParamaterizeCircle", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, false, 0, 0);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("ParamaterizeCircleCotan", [this, mesh, pOGLW = attr->pOGLW]() {
		auto paramaterize = Paramaterize::New(sobj, mesh, false, 0, 1);
		if (paramaterize->Run())
			printf("Paramaterize done\n");
		pOGLW->DirtyVAO(mesh);
	});
	...
	// ASAP
	grid->AddButton("ASAP", [this, mesh, pOGLW = attr->pOGLW]() {
		auto asap = ASAP::New(sobj, mesh, false);
		asap->Run();
		pOGLW->DirtyVAO(mesh);
	});
	// ASAP texture
	grid->AddButton("TexASAP", [this, mesh, pOGLW = attr->pOGLW]() {
		auto asap = ASAP::New(sobj, mesh, true);
		asap->Run();
		pOGLW->DirtyVAO(mesh);
	});
	int* p_debug = new(int);
	*p_debug = 5;
	grid->AddEditVal("-debug", *p_debug, 1, 100, 99);

	// ARAP
	int* p_iter_n = new(int);
	*p_iter_n = 5;
	grid->AddEditVal("-ARAP Iter Num", *p_iter_n, 1, 100, 99);
	grid->AddButton("ARAP", [p_iter_n, p_debug, this, mesh, pOGLW = attr->pOGLW]() {
		auto arap = ARAP::New(sobj, mesh, false);
		arap->RunARAP(*p_iter_n, 0.1, *p_debug);
		pOGLW->DirtyVAO(mesh);
	});
	grid->AddButton("ARAP_tex", [p_iter_n, p_debug, this, mesh, pOGLW = attr->pOGLW]() {
		auto arap = ARAP::New(sobj, mesh, true);
		arap->RunARAP(*p_iter_n, 0.1, *p_debug);
		pOGLW->DirtyVAO(mesh);
	});

	//grid->AddButton("DEBUG", [p_debug]() {
	//	if (*p_debug == false) *p_debug = true;
	//	else *p_debug = false;
	//});
